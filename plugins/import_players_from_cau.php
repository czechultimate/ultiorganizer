...
[DESCRIPTION]
title = "Import Players from CAU Evidence"
...
//season selection
$html .= "<form method='post' enctype='multipart/form-data' action='?view=plugins/import_players_from_cau'>\n";
...
function GetCAUTournaments($SeasonYear){
    ...
    $seasonData = json_decode(file_get_contents("$domain/list/season?filter[name]=$SeasonYear&token=$token"))->data;
    $seasonDataID = $seasonData[0]->id;
    $tournamentData = json_decode(file_get_contents("$domain/list/tournament?filter[season_id]=$seasonDataID&token=$token"))->data;
